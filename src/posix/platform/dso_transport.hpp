...
class DsoConnection : private NonCopyable
{
public:
    ~DsoConnection()
    {
    ...
    Error Connect(const otSockAddr *aPeerSockAddr)
    {
        ...
        if (IsIpv4Mapped(aPeerSockAddr->mAddress))
        {
            ...
            otLogInfoPlat("###### connecting to IPV4 mapped addr: %s %s", buf,
                          std::to_string(aPeerSockAddr->mPort).c_str());
        }

        if ((ret = mbedtls_net_connect(&mCtx, buf, std::to_string(aPeerSockAddr->mPort).c_str(),
                                       MBEDTLS_NET_PROTO_TCP)) != 0)
        {
            otLogWarnPlat("address: %s, port: %u", buf, aPeerSockAddr->mPort);
            otLogWarnPlat("mbedtls net connect failed: %s", MbedErrorToString(ret));
            error = kErrorFailed;
            ExitNow();
        }
...
//        mbedtls_ssl_conf_rng(&conf, mbedtls_ctr_drbg_random, nullptr);
//        mbedtls_ssl_conf_dbg(&conf, nullptr, stdout);

        mPeerSockAddr = *aPeerSockAddr;
        VerifyOrExit((ret = mbedtls_net_set_nonblock(&mCtx)) == 0, error = kErrorFailed);
        otLogInfoPlat("###### mbedtls net connect succeeded: %s", MbedErrorToString(ret));
    ...
    exit:
        if (error != kErrorNone)
        {
            otLogInfoPlat("???? mbedtls net connect failed: %s", MbedErrorToString(ret));
        }
        return error;
    }
    ...
    void Send(otMessage *aMessage)
    {
        ...
        otLogInfoPlat("Write: size = %hu", size);
        ...
        if (err < 0)
        {
            otLogWarnPlat("failed to send message: %s", MbedErrorToString(err));
        }
        else
        {
    ...
    void HandleReceive()
    {
        ...
        if (ret < 0)
        {
            ...
            {
                otLogWarnPlat("failed to receive message: %s", MbedErrorToString(ret));
            }
            ExitNow();
        }
        if (ret == 0)
        ...
        while (true)
        {
            if (!mPendingMessage)
            {
                ...
                {
                    mWantMessageSize =
                        Encoding::BigEndian::HostSwap16(*(reinterpret_cast<uint16_t *>(mBuffer + mBufferBegin)));
            ...
            otLogInfoPlat("Read: mWantMessageSize = %lu mBufferBegin = %hu mBufferEnd = %hu", mWantMessageSize,
                          mBufferBegin, mBufferEnd);
            int32_t readLen = OT_MIN(static_cast<int32_t>(mWantMessageSize) - otMessageGetLength(mPendingMessage),
                                     static_cast<int32_t>(mBufferEnd) - mBufferBegin);
            VerifyOrDie(readLen >= 0, 1);
            VerifyOrDie(otMessageAppend(mPendingMessage, mBuffer + mBufferBegin, readLen) == kErrorNone, 1);
            mBufferBegin += readLen;
            if (otMessageGetLength(mPendingMessage) == mWantMessageSize)
            {
                otLogInfoPlat("handle DSO receive: %lu", mWantMessageSize);
    ...
    void Disconnect(otPlatDsoDisconnectMode aMode)
    {
        ...
        case OT_PLAT_DSO_DISCONNECT_MODE_GRACEFULLY_CLOSE:
            ...
            break;
        default:
            otLogWarnPlat("unknown disconnection way");
            break;
        }
        mConnected = false;
        sMap.erase(mConnection);
...
private:
        ...
        , mCtx(aCtx)
    {
        ...
        otLogInfoPlat("!!! connection recorded !!!!");
    }
